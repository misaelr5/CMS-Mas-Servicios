...
function FlowCell({ text }: { text: string }) {
  ...
  const tone =
    dir === "in" ? "text-success" : dir === "out" ? "text-danger" : "text-lightGray/55";

  return (
    <span className="inline-flex items-center gap-1.5">
      <Icon className={cn("h-3.5 w-3.5 shrink-0", tone)} />
      <span className="text-brandWhite">{text}</span>
    </span>
  );
}
...
function BalanceCell({
  ...
  return (
    <div className="grid grid-cols-[auto_1fr] gap-x-3 gap-y-0.5 text-xs">
      <span className="text-lightGray/55">Efvo</span>
      <span className="text-right font-medium tabular-nums text-brandWhite">{formatArs(cash)}</span>
      <span className="text-lightGray/55">Cta</span>
      <span className="text-right font-medium tabular-nums text-brandWhite">{formatArs(account)}</span>
      <span className="text-lightGray/55">USD</span>
      <span className="text-right font-medium tabular-nums text-brandWhite">{formatUsd(usd)}</span>
    </div>
  );
}
  ...
  return (
    <span
      className={cn(
        ...
        value > 0 ? "text-success" : value < 0 ? "text-danger" : "text-lightGray/55"
      )}
    >
      {formatArs(value)}
    </span>
...
function AnnulInternalTransferForm({ operation }: { operation: BagOperation }) {
  ...
  return (
    <form action={annulInternalBagTransferFormAction} className="mt-3 grid gap-2 rounded-xl border border-danger/20 bg-danger/5 p-3">
      ...
      <input className="h-9 rounded-md border border-border bg-white/[0.06] px-3 text-xs text-brandWhite focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring" name="reason" placeholder="Motivo de anulación" required />
      <input className="h-9 rounded-md border border-border bg-white/[0.06] px-3 text-xs text-brandWhite focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring" name="note" placeholder="Nota obligatoria" required />
...
export function BagOperationsTable({
  ...
  if (operations.length === 0) {
    return <div className="rounded-md border border-white/10 bg-black/20 p-4 text-sm text-lightGray/55">Sin historial cargado para esta bolsa.</div>;
  }
    ...
    <div className="space-y-4">
      ...
      <div className="grid gap-3 lg:hidden">
        {operations.map((op) => {
          ...
          return (
            <article key={op.id} className="rounded-lg border border-white/10 bg-white/[0.06] p-4 text-sm text-brandWhite shadow-soft">
              <div className="flex items-start justify-between gap-3">
                <div>
                  <p className="font-heading text-base font-black">{operationLabel(op)}</p>
                  <p className="mt-1 text-xs text-lightGray/55">{new Date(op.created_at).toLocaleString("es-AR")}</p>
                </div>
              ...
              <div className="mt-4 grid gap-3 sm:grid-cols-2">
                <div className="rounded-md bg-black/20 p-3">
                  <p className="text-[11px] uppercase tracking-[0.16em] text-lightGray/55">Movimiento</p>
                  <p className="mt-1 font-semibold tabular-nums">{formatArs(Number(op.total_ars ?? 0))}</p>
                  <p className="text-lightGray/55">{formatUsd(Number(op.amount_usd ?? 0))} USD a {formatArs(Number(op.rate_ars ?? 0))}</p>
                </div>
                <div className="rounded-md bg-black/20 p-3">
                  <p className="text-[11px] uppercase tracking-[0.16em] text-lightGray/55">Flujo</p>
                  <p className="mt-1 font-semibold"><FlowCell text={flow} /></p>
                  <p className="mt-1 text-lightGray/55">Ganancia: <GananciaValue value={Number(op.profit_ars ?? 0)} /></p>
                </div>
                <div className="rounded-md bg-black/20 p-3">
                  <p className="text-[11px] uppercase tracking-[0.16em] text-lightGray/55">Saldo anterior</p>
                  <div className="mt-1">
                    <BalanceCell
                ...
                <div className="rounded-md bg-black/20 p-3">
                  <p className="text-[11px] uppercase tracking-[0.16em] text-lightGray/55">Saldo nuevo</p>
                  <div className="mt-1">
                    <BalanceCell
              ...
              {op.notes ? <p className="mt-3 rounded-md bg-brandYellow/15 p-3 text-sm text-brandWhite">Nota: {op.notes}</p> : null}
      ...
      <div className="hidden overflow-hidden rounded-lg border border-white/10 lg:block">
        <div className="overflow-x-auto">
          <table className="min-w-full text-left text-sm">
            <thead>
              <tr className="border-b border-white/10 bg-black/20 text-[11px] uppercase tracking-[0.12em] text-lightGray/55">
            ...
            <tbody className="divide-y divide-white/10 bg-white/[0.06] text-brandWhite">
              {operations.map((op) => {
                ...
                return (
                  <tr key={op.id} className="transition-colors hover:bg-brandYellow/5">
                    <td className="whitespace-nowrap px-4 py-3.5 align-top">
                      <p className="font-semibold text-brandWhite">{created.toLocaleDateString("es-AR")}</p>
                      <p className="text-xs text-lightGray/55">{created.toLocaleTimeString("es-AR", { hour: "2-digit", minute: "2-digit" })}</p>
                    </td>
                    ...
                    <td className="max-w-[200px] px-4 py-3.5 align-top">
                      {op.notes ? <span className="text-brandWhite">{op.notes}</span> : <span className="text-lightGray/55/70">Sin nota</span>}
                    </td>
                    {canManageInternalTransfers ? (
                      <td className="px-4 py-3.5 align-top">
                        {canAnnulInternalTransfer(op, canManageInternalTransfers) ? <AnnulInternalTransferForm operation={op} /> : <span className="text-xs text-lightGray/55/70">Sin acciones</span>}
                      </td>
                    ) : null}
                  </tr>
                );
